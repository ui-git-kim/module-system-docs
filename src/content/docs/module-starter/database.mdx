...
model my_feature_item {
  ...
  // Compound index supports the default "items per user, newest first"
  // list query without an in-memory sort. Add additional compound indexes
  // for other common filter combinations (e.g. [userId, status]).
  @@index([userId, createdAt])
}
