...
Recent scaffold versions add significant module-authoring features — review the [Changelog](/module-starter/changelog/) if you last cloned before v1.7.0:
...
- **v1.7.0** — Backend list endpoints paginated, atomic `updateMany`/`deleteMany` ownership check, compound `[userId, createdAt]` index, React render fixes
- **v1.6.0** — `STYLE_REGISTRATION` template for runtime CSS + shadcn `components.json` contributions
- **v1.5.0** — `update` command auto-installs new deps, shadcn components, and env variables
</Aside>
