...
|---------|-----------|--------------|
| **Module Version** | `package.json` + `src/module.ts` | After changing templates or module config |
| **Scaffold Version** | `VERSION` file | After changing scaffold infrastructure (rare) |

```bash
# After template changes — bumps both files and auto-commits
...
<Aside type="caution">
**Always bump the version** after changing templates. Every generated file gets a version header — the update command uses this to detect changes. Without a bump, downstream apps won't see your updates.
</Aside>

For the full explanation of how updates propagate, version headers, scaffold upstream commands, and what gets updated vs preserved, see **[Versioning & Updates](/module-starter/versioning/)**.
