---
title: Module Starter Overview
description: Create distributable module packages for the starter template
---

import { Aside, Steps } from '@astrojs/starlight/components';

The **Module Starter** is a template repository for creating installable module packages. When you clone it and customise the templates, you get a CLI tool that can scaffold fullstack features into apps built with the [Starter Template](/starter-template/).

<Aside type="tip">
The module starter creates **npm packages** with CLI commands like `npx my-module init`. It's not just a folder structure — it's a distribution system.
</Aside>

<Aside type="note" title="What's new">
Recent scaffold versions add significant module-authoring and tooling features — review the [Changelog](/module-starter/changelog/) if you last cloned before v1.10.0:

- **v1.13.0** — CI-safe `scaffold:update` (`--yes`/CI, never hangs); self-update commits **locally** (opt-in `--push-self-update`); module version bumped only when templates changed; cross-platform `run.cjs` (`pwsh → powershell.exe → bash`); new `parity:check`
- **v1.12.0** — `src/templates/**` + `src/module.ts` are **module-owned** (`@scaffold-reference`); `scaffold:update` keeps them and only auto-adds brand-new template files
- **v1.11.0** — Security integration: every backend module ships `security.registration.ts` (manifest + account-deletion handler)
- **v1.9.0** — `QUERY_HOOKS_TEMPLATE` scaffolds TanStack Query hooks (`hooks/useMyFeature.ts`) alongside the service
- **v1.8.0** — Settings page uses debounced live-save with `AbortController` cancellation
- **v1.7.0** — Backend list endpoints paginated, atomic ownership checks, compound `[userId, createdAt]` index
</Aside>

---

## How It Works

<Steps>
1. **Clone** the module starter using the create script
2. **Customise** the templates in `src/templates/`
3. **Build** with `npm run build`
4. **Distribute** via npm or direct installation
5. **Install** into apps using `npx my-module init`
</Steps>

The module starter handles all the scaffolding logic — you just write templates.

---

## Package Structure

The module starter has two distinct parts: **scaffold infrastructure** (shared, don't edit) and **your templates** (customise these).

```
module-starter/
├── src/
│   ├── module.ts           # Module configuration (EDIT THIS)
│   ├── cli.ts              # CLI entry point (DO NOT EDIT)
│   ├── scaffold/           # Scaffolding infrastructure (DO NOT EDIT)
│   │   ├── init.ts         # Installation logic
│   │   ├── update.ts       # Update logic
│   │   ├── uninstall.ts    # Uninstall logic
│   │   └── ...
│   └── templates/          # Your templates (EDIT THESE)
│       ├── schema.ts       # Prisma schema templates
│       ├── frontend/       # Frontend file templates
│       └── backend/        # Backend file templates
├── package.json            # Module package config
├── VERSION                 # Scaffold infrastructure version
└── CLAUDE.md               # AI assistant instructions
```

For the full file reference, see [File Reference](/module-starter/file-reference/).

---

## Key Concepts

### Registry Pattern

Generated code integrates with the app through **registries** — singleton objects that modules register with instead of editing shared files. Your module registers navigation items, dashboard widgets, settings pages, API routes, services, and more — all without touching the app's core code.

For how this connects to the starter template, see [Starter Integration](/module-starter/starter-integration/). For practical registration examples, see [Registry Guide](/module-starter/registry-guide/).

### Self-Contained Schemas

Module Prisma schemas must have **no `@relation` directives**:

```prisma
// CORRECT - self-contained
model ai_chat_message {
  id        String   @id @default(cuid())
  userId    String                          // Plain FK, not a relation
  content   String
  createdAt DateTime @default(now())

  @@index([userId])
}
```

---

## CLI Commands

After building your module, these commands become available:

```bash
npx my-module init              # Install module into project
npx my-module init --force      # Overwrite existing files
npx my-module update            # Update to latest templates and schema
npx my-module update --dry-run  # Preview update changes
npx my-module update --skip-schema  # Update templates only
npx my-module migrate           # Add version headers to old files
npx my-module uninstall         # Remove module from project
npx my-module disable           # Disable without removing files
npx my-module enable            # Re-enable a disabled module
npx my-module info              # Show module information
```

For detailed command documentation with flags, output examples, and error handling, see [CLI Reference](/module-starter/cli-reference/).

---

## Versioning

The module system tracks two versions:

| Version | Stored In | When to Bump |
|---------|-----------|--------------|
| **Scaffold Version** | `VERSION` file | After **any** change in the module starter repo |
| **Module Version** | `package.json` + `src/module.ts` | After changing templates in a **cloned module** |

<Aside type="caution">
**In the module starter repo, all changes are scaffold bumps.** The module version bump is only used in cloned modules. See [Versioning & Updates](/module-starter/versioning/) for the full explanation.
</Aside>

---

## Quick Reference

| What | Where |
|------|-------|
| Module configuration | `src/module.ts` |
| Frontend templates | `src/templates/frontend/` |
| Backend templates | `src/templates/backend/` |
| Schema template | `src/templates/schema.ts` |
| Placeholder replacement | `src/scaffold/utils.ts` → `processTemplate()` |
| Type definitions | `src/scaffold/types.ts` |
| Installation logic | `src/scaffold/init.ts` |
| Installation manifest | `.module-manifests.json` (in target project) |
| Module state (enabled flag, prompt answers, installed assets for delta updates) | `.module-state.json` (in target project) |

---

## Next Steps

- [Create Script Walkthrough](/module-starter/create-script/) — Clone and set up a new module
- [Building Your Module](/module-starter/building-modules/) — Customise templates and configuration
- [Code Templates](/module-starter/templates/) — Copy-paste templates for all registries
- [CLI Reference](/module-starter/cli-reference/) — Full command documentation with flags and examples
- [Lifecycle Hooks](/module-starter/lifecycle-hooks/) — Module lifecycle, filters, and actions
- [Module Database](/module-starter/database/) — Schema patterns, transactions, and self-contained design
- [File Reference](/module-starter/file-reference/) — File structure and naming conventions
- [Scaffold Architecture](/module-starter/scaffold/) — How the scaffold infrastructure works
- [Starter Integration](/module-starter/starter-integration/) — How modules connect to the starter template
- [Registry Guide](/module-starter/registry-guide/) — Register with the starter template's registries
- [Versioning & Updates](/module-starter/versioning/) — How changes propagate through the system
- [Developer Workflow](/module-starter/developer-workflow/) — Working on the module starter itself
