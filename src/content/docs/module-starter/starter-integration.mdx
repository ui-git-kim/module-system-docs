...
import './navigation.registration';                // Side-effect: registers nav items
import './registrations/dashboard.registration';   // Side-effect: registers a dashboard widget
import './registrations/preferences.registration'; // Side-effect: registers a preferences section

export { MyFeaturePage } from './pages/MyFeaturePage';
export { MyFeatureWidget } from './components/widgets/MyFeatureWidget';
```

The registration files are split across two locations. Under `registrations/`: `dashboard`, `preferences`, `settings-page`, `integrations`, `personal-info`, `notification-preferences`, `language-options`, and `style`. At the feature **root**: `navigation`, `admin-pages`, `header-toolbar`, `filter`, and `action`. (Backend barrels use the same pattern with `.js` extensions — see [Backend Registries](#backend-registries) below.)
...
| Notification Preferences | `@/config/notification-preferences.registry` | `register()` | Notification toggles |
| Language Options | `@/config/language-options.registry` | `register()` | Language selector options |
| Style | `@/config/style.registry` | `register()`, `update()` | Runtime CSS tokens + shadcn `components.json` contributions |
...
| Services | `../../config/service.registry.js` | `register()` (`registerDynamic()` lives in the starter template for lazy/plugin services) | Business logic access |
| Security | `../security/index.js` | `register()`, `registerDeletionHandler()` | Ownership / encryption / deletion-cascade manifest |
| Module | `../modules/index.js` | `register()` | Runtime module registry (installed-module state sync) |
...
<Aside type="note">
The **Security** registry is shipped by default — every backend module generates a `security.registration.ts` (side-effect imported from `index.ts`) declaring a security manifest and an account-deletion handler. Note it imports from `../security/index.js`, **not** `../../config/...`. Frontend-only or no-per-user-data modules can delete the file (and its `index.ts` import). See [Security](/module-starter/security/).
</Aside>
...
"What a module is" — `id`, `name`, `description`, `version`, `depends`, lifecycle hooks — is defined by the **starter template**, not the scaffold: the foundation `ModuleContract` lives in the starter's [modules feature](/starter-template/modules-feature/) (`backend/src/features/modules/module-contract.ts`, starter v1.48.0+) and the scaffold consumes a hash-stamped synced copy (`src/scaffold/module-contract.ts`). Your `ModuleDefinition` extends that contract with the scaffold's codegen configuration.

At runtime, the generated `module.registration.ts` registers `{ id, name, version, depends }` with the host's `moduleRegistry` — the starter's startup state sync records install/update/missing transitions in its `InstalledModule` table and emits the module lifecycle audit events. On starters older than v1.48.0 the registration degrades to a no-op (guarded dynamic import) instead of crashing the boot.
...
| Notification preferences registry API | `src/templates/frontend/registrations.ts` |
| Language options registry API | `src/templates/frontend/registrations.ts` |
