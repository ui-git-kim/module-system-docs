...
Navigation lives at the feature root as `navigation.registration.ts`; collection registrations (dashboard, preferences, settings page, etc.) live under `registrations/`. (Backend barrels use the same pattern with `.js` extensions — see [Backend Registries](#backend-registries) below.)
