...
<Aside type="caution" title="Exception: src/scaffold/module-contract.ts is starter-owned">
`src/scaffold/module-contract.ts` is the one scaffold file you must **not** edit by hand — it is a verbatim copy synced from the starter template's canonical `backend/src/features/modules/module-contract.ts`. Refresh it with `npm run contract:sync` (scaffold repo only) rather than editing it directly. The scaffold version bump runs `sync-contract.cjs --check` and **fails the release** on any hand-edit or drift from the starter. To change the contract, edit it in the starter repo, then run `contract:sync` here. CLI-only lifecycle hooks that don't belong in the shared contract go in `src/scaffold/types.ts` instead.
</Aside>
...
```bash
# After ANY code change in this repo — the cadence is PATCH-DEFAULT
npm run scaffold:version:patch    # The default — almost everything: fixes, new templates, new commands, new features, tooling, removals, docs alignment
npm run scaffold:version:minor    # RARE — a genuinely new subsystem or registry (a headline release)
npm run scaffold:version:major    # A deliberate breaking epoch / post-launch migration
```

<Aside type="note">
**Patch-default.** A new template or CLI command on its own is a **patch**, not a minor — if you're reaching for `minor` because a change "adds a feature", it's almost certainly a patch. See [Versioning → Cadence](/module-starter/versioning/#version-bump-commands).
</Aside>
...
| Change | Bump? | Level |
|--------|-------|-------|
| `src/templates/**/*.ts` | Yes | `patch` (a new template on its own is still a patch) |
| `src/scaffold/**/*.ts` | Yes | `patch` (a new feature/command on its own is still a patch) |
| `src/module.ts` | Yes | `patch` |
| `src/cli.ts` | Yes | `patch` (a new CLI command on its own is still a patch) |
| `scripts/*` | Yes | `patch` |
...
| 4. Bump module version | Auto (since v1.13.0) | `scaffold:update` auto-bumps when the pull changed `src/templates`/`src/module.ts`; otherwise `npm run version:patch` for an infra-only pull |
| 5. Install/update in app | App developer | `npx my-module init` or `npx my-module update` |

<Aside type="note">
Since v1.12.0, `src/templates/**` and `src/module.ts` are **module-owned**: `scaffold:update` does **not** merge upstream changes into a module's existing template files — it only auto-adds brand-new ones and reports edits to existing files as an **advisory** diff (`npm run scaffold:check`) to hand-port. Only **infrastructure** (`src/scaffold/**`, `src/cli.ts`, `scripts/**`, config) is merged. A module developer who has customised their templates hand-ports the changes they want, rather than resolving a merge.
</Aside>
...
2. **Push the docs submodule:**
   ...
   git pull --rebase origin main    # Pull any upstream commits first
   git push
   cd ..
   ```
...
<Aside type="caution">
The docs submodule is shared across three repos (starter template, module starter, individual modules). Expect upstream commits to land between local commit and push — always `git pull --rebase` before `git push` in the submodule, or you will hit a non-fast-forward rejection. Conflicts are usually easy: they land in shared pages like `llm-rules.mdx` or cross-cutting sidebar entries.
</Aside>
...
- [ ] **Placeholders used** — Templates use `my-feature`, `MyFeature`, `myFeature`, `my_feature`, `MY_FEATURE`, `My Feature`, `my feature` patterns
...
- [ ] **`.ps1`/`.sh` parity** — If you changed `update-scaffold.*` or a `scripts/*` pair, run `npm run parity:check` to confirm both variants stay in sync
...
|-------------|--------|------|
| New template file | Low — auto-added to all modules on `scaffold:update` | Low |
| Modified template (existing file) | Low–Medium — module-owned, so existing modules get it as an **advisory** to hand-port; new modules (`create-module`) get it directly | Low |
| New scaffold feature | Medium — available to all modules after update | Low |
| New scaffold **file** (`src/scaffold/**`, `scripts/*`) | Low — merged in, and backfilled on the next update for any module that never received it (v1.30.0+) | Low |
