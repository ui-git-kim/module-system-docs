...
The module starter ships with template files for the most common patterns (pages, routes, services, registrations, etc.), but bigger modules often need more. You can add your own template files for hooks, utilities, dialogs, context providers — anything your module needs.

### Creating a New Template File

<Steps>
1. **Create the file** in the appropriate templates directory:
...
2. **Export from the index** — add your new file to `src/templates/frontend/index.ts`:
...
3. **Reference in module.ts** — add the templates to your frontend config:
...
- **One file per category** — group related templates together (e.g., all hooks in `hooks.ts`, all dialogs in `dialogs.ts`). Split into separate files if a single file gets too large.
...
- **Backend uses `.js` imports** — any relative import in a backend template must use `.js` extension for ESM compatibility.

<Aside type="tip">
The template files themselves are just TypeScript files that export string constants. There's no limit to how many you can create — organise them however makes sense for your module. The `index.ts` barrel file re-exports everything so `module.ts` can import with `import * as frontend from './templates/frontend/index.js'`.
</Aside>
