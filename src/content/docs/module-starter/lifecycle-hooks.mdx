...
Modules can hook into application lifecycle events using **filters** (transform data) and **actions** (execute side effects). These registries enable modules to extend functionality without modifying core code.
...
| `registerAsync(hook, id, fn, order?)` | Register async filter (backend) |
| `unregister(hook, id)` | Remove a filter |
| `apply(hook, value, ctx?)` | Run sync filters |
| `applyAsync(hook, value, ctx?)` | Run async filters |
| `applyAll(hook, value, ctx?)` | Run sync then async |
| `hasFilters(hook)` | Check if hook has filters |
...
| `doActionAsync(hook, context)` | Execute async actions |
| `hasActions(hook)` | Check if hook has actions |
...
import { filterRegistry, actionRegistry } from '@/config/filter.registry';
