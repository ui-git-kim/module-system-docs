...
- For I/O, use `registerAsync` / `addActionAsync`. The starter awaits registered async work at `doActionAll`, so prefer `doAction` (no await) for fire-and-forget semantics
