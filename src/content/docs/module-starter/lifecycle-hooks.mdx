...
Lifecycle hooks let your module run code at key moments in its lifecycle. Define them in your `module.ts`:

```typescript
lifecycle: {
  onInstall: async (context) => { /* Seed data, create indexes */ },
  onUpdate: async (context) => { /* Run migrations between versions */ },
  onActivate: async (context) => { /* Start jobs, register webhooks */ },
  onDeactivate: async (context) => { /* Pause jobs, cleanup */ },
  onUninstall: async (context) => { /* Export data, remove external resources */ },
},
```
...
All hooks receive a context object with utilities for file operations, user prompts, and logging. Install and update hooks receive additional context (prompt answers, version info). See [Scaffold Architecture](/module-starter/scaffold/#module-lifecycle-hooks) for the full context API.
...
Modules can also hook into **runtime** application events using **filters** (transform data) and **actions** (execute side effects). These are pipeline registries that enable modules to extend functionality without modifying core code.
...
- [Scaffold Architecture](/module-starter/scaffold/) - Full lifecycle context API and hook details
