...
// FILE: frontend/src/features/my-feature/services/myFeature.service.ts
// PURPOSE: API service for my-feature
// ========================================================================

import { api } from '@/lib/api';
import type { MyFeatureItem, CreateMyFeatureInput, UpdateMyFeatureInput } from '../types';

const BASE_URL = '/api/v1/my-feature';

export const myFeatureService = {
  async getAll(): Promise<MyFeatureItem[]> {
    const { data } = await api.get(BASE_URL);
    return data;
  },

  async getById(id: string): Promise<MyFeatureItem> {
    const { data } = await api.get(\`\${BASE_URL}/\${id}\`);
    return data;
  },

  async create(input: CreateMyFeatureInput): Promise<MyFeatureItem> {
    const { data } = await api.post(BASE_URL, input);
    return data;
  },

  async update(id: string, input: UpdateMyFeatureInput): Promise<MyFeatureItem> {
...
export const myFeatureService = {
  ...
  async create(userId: string, data: CreateMyFeatureInput) {
    const item = await prisma.my_feature_item.create({
      data: { userId, ...data },
    });
    log.info({ userId, itemId: item.id }, 'Item created');
    await actionRegistry.doAction('my-feature.item.created', { userId, item });
    return item;
  },

  async update(id: string, userId: string, data: UpdateMyFeatureInput) {
    await this.getById(id, userId);
    const item = await prisma.my_feature_item.update({
      where: { id },
      data,
    });
    log.info({ userId, itemId: item.id }, 'Item updated');
    await actionRegistry.doAction('my-feature.item.updated', { userId, item });
    return item;
  },
  ...
  async delete(id: string, userId: string) {
    ...
    await actionRegistry.doAction('my-feature.item.deleted', { userId, itemId: id });
  },
};
`;
```
