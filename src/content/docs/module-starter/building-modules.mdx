...
- Settings components like `SettingsComponents` live in the starter at `@/features/user/components/settings/`
...
Import from `@/features/user/components/settings/SettingsComponents`:

| Component | Purpose |
|-----------|---------|
| `SettingsRow` | Two column: label left, action right |
| `SettingsRowStacked` | Content below label, action top right |
| `SettingsToggleRow` | Label with toggle switch |
| `SettingsDropdownRow` | Label left, dropdown right (aligned) |
| `SettingsSectionHeader` | Section title with optional action |
| `SettingsDivider` | Dashed horizontal divider |
| `SettingsNote` | Info box for additional context |
| `SettingsDangerBox` | Red box for destructive actions |
| `SettingsPlanCard` | Marketing-style plan display |
| `SettingsProviderCard` | Card for connected accounts/services |
| `SettingsTable` | Table for sessions, invoices, etc. |

➡️ **[User Feature](/starter-template/user-feature/)** — Full component reference with props and examples
...
<Aside type="tip">
As the starter template grows, more reusable components will be added (admin components, etc.). Always check the [starter template feature docs](/starter-template/features/) for the latest available components.
</Aside>
