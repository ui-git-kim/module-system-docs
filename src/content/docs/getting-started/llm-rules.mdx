...
|-------|-------------|----------|
| **Patch** | Bug fixes, typo corrections, minor adjustments that don't change behaviour | Fix a typo in error message, correct a CSS class, fix a broken import |
| **Minor** | New features, enhancements, refactors that don't break existing code | Add new registry method, new component, restructure without API changes |
| **Major** | Breaking changes that require apps to update their code | Remove/rename API endpoints, change type signatures, remove exported functions |

<Aside type="tip">
  **Default to patch.** Most changes are patch-level. Only use minor for genuinely new capabilities, and major for changes that would break existing apps.
</Aside>
