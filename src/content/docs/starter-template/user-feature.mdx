...
| Billing | `/settings/billing` | Subscription (placeholder) | 50 |
...
The `DELETE` handler responds with `{ success, message, auditId }` and runs the provider-delegated deletion cascade (see `features/security`), passing the request IP and User-Agent for audit logging. The frontend `deleteAccount()` ignores the body and returns `void`.
