...
Use `signOutAndClear()` — it calls `authClient.signOut()`, then clears the TanStack Query cache in a `finally`. That frees the previous account's cached profile (including a base64 avatar), notifications, and settings immediately, and closes a same-tab privacy gap where the old data would otherwise linger (until `gcTime`) and be briefly readable after a user switch. The starter wires it into `NavUser` and the `DataPrivacyPage`. Callers own any navigation afterwards.

```tsx
import { signOutAndClear } from '@/features/auth';

async function handleSignOut() {
  await signOutAndClear();       // signOut() + queryClient.clear()
...
4. **MFA + step-up via Neon Auth, by tier.** MFA is available at `standard`, enforced for admins at `hardened`, mandatory at `strict`; step-up re-auth for sensitive actions at `hardened+`. All configured on the Neon side, surfaced by `securityProfile` — see the [Security feature](/starter-template/security-feature/).
