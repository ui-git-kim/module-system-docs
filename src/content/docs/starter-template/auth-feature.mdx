...
Requires the user to have the `admin` role. Queries the database to check:
...
routeRegistry.register({
  ...
  router: adminRoutes,
  feature: 'admin',
  requiresAuth: true,  // requireAuth applied automatically
  middleware: [requireAdmin],  // additional middleware
});
```
...
If your module has admin endpoints, add the `requireAdmin` middleware:
