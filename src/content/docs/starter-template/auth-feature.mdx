...
In `main.tsx`, the app is wrapped with `NeonAuthUIProvider` (auth context) and then with `TooltipProvider` (so any shadcn tooltip in the tree works without its own wrapper — added in v1.20.2):
