...
Priority determines the order widgets appear in the grid:

| Range | Purpose |
|-------|---------|
| 1-20 | Core/system widgets |
| 21-50 | Primary feature widgets |
| 51-80 | Secondary widgets |
| 81-99 | Utility widgets |
...
The `DashboardGrid` component renders widgets in a responsive CSS grid:

```tsx
import { DashboardGrid } from '@/features/dashboard/components/DashboardGrid';

function DashboardHome() {
  const widgets = useVisibleDashboardWidgets(preferences);
...
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
  {widgets.map(widget => (
    <div
      key={widget.id}
      className={getSpanClasses(widget.size)}
    >
      <widget.component widget={widget} isCompact={false} />
    </div>
  ))}
</div>
```
...
function MyComponent() {
  const allWidgets = useDashboardRegistry();
  // Returns all registered widgets, sorted by priority
}
```
...
2. **Sensible priorities** - Use the priority ranges consistently
