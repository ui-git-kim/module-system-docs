...
<Aside type="tip">
  For details on the navigation layouts (sidebar or top bar), header, navigation, and toolbar, see [Shell Layout](/starter-template/shell-layout/).
</Aside>
...
- **Application Shell** - Side (sidebar) or top (top bar) navigation, page header, and content area (see [Shell Layout](/starter-template/shell-layout/))
...
├── navigation.registration.ts      # Registers "Dashboard" in the main navigation
...
│   ├── header/                     # Page header: breadcrumbs + toolbar (both layouts)
│   ├── shell/                      # AppShell (side/top switch), AppBrand, breadcrumbs, user menu
│   ├── sidebar/                    # Sidebar components (side layout block)
│   ├── topbar/                     # Top bar components (top layout block)
│   └── widgets/
│       └── ExampleWidgets.tsx      # Example widget components
├── hooks/
│   └── useResolvedNavigation.ts    # Registry + admin overrides + roles → nav (both layouts)
└── lib/
    ├── resolve-navigation.ts       # Pure navigation resolver (unit-tested)
    └── close-on-link-click.ts      # Closes the mobile nav sheets on navigation
```
    ...
    <div className="grid grid-cols-3 gap-4">
      {stats.map((stat) => (
        <div key={stat.label} className="text-center">
          <p data-slot="widget-stat-value">{stat.value}</p>
...
- [Shell Layout](/starter-template/shell-layout/) - Sidebar / top bar, header, navigation details
