...
| `npm run deps:check` / `deps:update` / `deps:update:latest` | Check or apply dependency updates across root/frontend/backend | [Dependency Updates](#dependency-updates) |
...
Keep the template's root, frontend and backend dependencies up to date. Three modes cover the common cases from a quick peek to a full major-version sweep.

<Aside type="caution">
  **Run these in a cloned app, not in the template repo itself.** The template has no `node_modules`. After running `deps:update:latest`, test thoroughly — major version jumps can introduce breaking changes.
</Aside>
...
| Mode | Respects semver? | Tool | Changes lockfiles? |
|------|------------------|------|--------------------|
| `deps:check` | n/a (read-only) | `npm outdated` + `npx npm-check-updates` | No |
| `deps:update` | Yes — won't cross majors | `npm update` | Yes |
| `deps:update:latest` | **No** — jumps to latest | `npx npm-check-updates -u` + `npm install` | Yes |
...
<Tabs>
  <TabItem label="Windows">
    ```powershell
    .\scripts\update-deps.ps1              # Safe update
    .\scripts\update-deps.ps1 -Check       # Preview only
    .\scripts\update-deps.ps1 -Latest      # Bump to latest (may break)
    ```
  </TabItem>
  <TabItem label="macOS/Linux">
    ```bash
    ./scripts/update-deps.sh               # Safe update
    ./scripts/update-deps.sh --check       # Preview only
    ./scripts/update-deps.sh --latest      # Bump to latest (may break)
    ```
  </TabItem>
</Tabs>
...
- scripts/
  ...
  - update-deps.ps1 (Dependency updates)
