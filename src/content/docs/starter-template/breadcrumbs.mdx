...
A page that sets **dynamic** breadcrumbs (via `useBreadcrumbs`) is responsible for clearing them on unmount — return `() => setBreadcrumbs(null)` from the effect. The `BreadcrumbProvider` deliberately does **not** auto-clear on route change: in React a parent effect runs *after* child effects, so clearing in the provider would wipe the incoming page's breadcrumbs. Automatic (registry-driven) breadcrumbs need no cleanup.
...
function ProjectDetailPage() {
  ...
  useEffect(() => {
    ...
    // Clear on unmount — the provider does not auto-clear on navigation.
...
- Clear dynamic breadcrumbs on unmount (`return () => setBreadcrumbs(null)`) — the provider does not auto-clear on navigation
...
| Hook | Description |
|------|-------------|
| `useBreadcrumbs()` | Returns `{ dynamicBreadcrumbs, setBreadcrumbs }` — read the current crumbs, or set the page's (clear on unmount) |
