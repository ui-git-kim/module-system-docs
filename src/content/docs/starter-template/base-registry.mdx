...
If you don't specify `order`, the registry uses its default of **50** (middle of the Feature range). Items with the same order appear in registration order.

```typescript
// No order specified - uses default of 50
navigationRegistry.registerMain({ id: 'feature-a', title: 'Feature A', ... });
navigationRegistry.registerMain({ id: 'feature-b', title: 'Feature B', ... });

// Both have order 50, so Feature A appears first (registered first)
```

### Order Ranges (Developer Defaults)
...
<Aside type="tip">
The default order of **50** places unordered items at the bottom of the Feature range. Set an explicit order to control position precisely.
</Aside>

### Admin Overrides

**Order numbers are only for developers** setting sensible defaults. Admins never see these numbers.

In the admin panel, admins can:
- **Reorder items visually** (drag and drop)
- **Hide items** they don't want shown

Admin overrides are stored in the database and take precedence over developer defaults. So if a developer sets `order: 40` but an admin drags that item to the top, the admin's choice wins.

Under the hood, the frontend `BaseRegistry` exposes three methods for this. At bootstrap, `applyAdminOverrides(hidden: string[])` loads the persisted set of hidden IDs (usually from `AppSettingsContext`) and notifies subscribers. `getActive()` then returns the registered items minus anything an admin has hidden — use it where overrides should apply (e.g. a dashboard's rendered widgets), and use `getAll()` where every item should be listed regardless (e.g. an admin management UI). `isAdminHidden(id)` reports whether a single item is hidden. These admin methods are frontend-only.
...
unregisterMany(ids)           // Remove multiple by ID
clear()                       // Remove all
```
...
getActive()  // Get all items except admin-hidden (frontend only)
...
No React, same core registration API:
- Same `register()`, `getAll()`, `unregisterMany()`, etc.
- Used for routes, services
- No admin-override API — `getActive()`, `applyAdminOverrides()`, and `isAdminHidden()` are frontend-only
- Internally uses a lazy `sortedCache` (invalidated on mutation) instead of the frontend's eagerly-maintained snapshot, and logs duplicate-registration warnings through the pino module logger (`log.warn`) rather than `console.warn`
