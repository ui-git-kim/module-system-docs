...
<Aside type="note">
This single endpoint returns **everything** users need: app name, logo, navigation overrides, section overrides, and toolbar overrides. This is what the `AppSettingsProvider` fetches for all users to render the sidebar correctly.
</Aside>
...
<Aside type="tip">
The admin panel also uses GET endpoints for `/navigation`, `/section-overrides`, and `/toolbar` to fetch data for editing. These are admin-only because regular users get everything from the single `/settings` endpoint.
</Aside>
