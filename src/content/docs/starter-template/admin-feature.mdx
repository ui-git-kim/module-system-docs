...
- **Sidebar Sections** - Organise navigation items into sections
...
│   ├── SidebarSectionsPage.tsx     # Section organisation
...
| `useSidebarSections()` | Sidebar sections and item assignments |
...
Admins can organise navigation items into custom sections, providing a more structured sidebar experience.

### Creating Sections

<Steps>
1. Go to Admin → Sidebar Sections
2. Enter a section name in the "New section name" field
...
<Steps>
1. Go to Admin → Sidebar Sections
2. In the "Item Assignments" card, use the dropdown for each navigation item
3. Select the section you want to assign it to
...
| Feature | Description |
|---------|-------------|
| **Custom labels** | Each section can have its own label (e.g., "Reports", "Settings") |
| **Label visibility** | Toggle whether the section label is shown in the sidebar |
| **Drag-and-drop ordering** | Reorder sections by dragging |
| **Item counts** | See how many items are assigned to each section |
...
- Items without an assignment default to "main"
...
interface SidebarSections {
  ...
  itemAssignments: Record<string, string>; // navItemId → sectionId
}
```

### Using Sidebar Sections in Components

```tsx
import { useSidebarSections } from '@/features/admin';

function MyComponent() {
  const sidebarSections = useSidebarSections();

  // Access sections
  console.log(sidebarSections.sections);

  // Check item assignments
  const itemSection = sidebarSections.itemAssignments['my-item-id'] || 'main';
}
```
...
| `/api/v1/admin/sidebar-sections` | GET | Get sidebar sections config |
| `/api/v1/admin/sidebar-sections` | PATCH | Update sidebar sections |
