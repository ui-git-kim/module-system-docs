...
| `defaultOrder` | `number` | `50` | Default order for items that don't specify one |
...
Get all registered items except those an admin has globally disabled, sorted by `order`. Use where admin overrides should apply (e.g. rendered widgets); use `getAll()` where every item should be listed regardless (e.g. an admin management UI). Frontend only.
...
```typescript
const visibleWidgets = dashboardRegistry.getActive();
```
...
These public methods let an admin globally disable registry items (e.g. hide a dashboard widget or navigation entry for everyone). Override IDs are typically applied once at bootstrap from persisted app settings. These are **frontend only** — the backend `BaseRegistry` has no admin-override support.

### getActive()

Get all registered items except those an admin has globally disabled, sorted by `order`. See [Retrieval Methods](#getactive) above.

```typescript
getActive(): T[]
```

### applyAdminOverrides(hidden)

Replace the set of admin-disabled IDs and notify subscribers so the UI re-renders. Pass an empty array to clear all overrides.

```typescript
applyAdminOverrides(hidden: string[]): void
```

```typescript
// At bootstrap, from persisted app settings
dashboardRegistry.applyAdminOverrides(['analytics-overview']);
```

### isAdminHidden(id)

Check whether an item is globally disabled by an admin.

```typescript
isAdminHidden(id: string): boolean
```
...
All registries follow a consistent ordering convention on a 0–100 scale. Lower numbers appear (or execute) first.
...
The default order is **50** (the middle of the Feature range), and values are capped at **100**. Admin overrides stored in the database take precedence over these defaults.
