...
interface ToolbarItem extends RegistryItem {
  ...
  inTopBar?: boolean;          // Top nav layout: render in the top bar, not the page header (v2.12.0)
  onClick?: () => void;
...
**Import:** `@/config/style.registry`

**Item type:** `StyleRegistryEntry`

```typescript
interface StyleRegistryEntry extends RegistryItem {
  id: string;                              // e.g. 'style.shadcn-config'
  order?: number;                          // Default: 50 (later entries win on token overlap)
  enabled?: boolean;                       // Module default; admins override via AppSettings. Default: true
  tokens?: Record<string, string>;         // CSS custom properties applied to :root
  css?: string;                            // Raw stylesheet text injected into <style id="module-styles">
  componentsJson?: ComponentsJsonContribution; // Mirror of the module's components.json contribution
}
```

The `register`, `update` and `unregister` methods are overridden to enforce exclusivity (`componentsJson.style`, `tailwind.baseColor` and `tailwind.cssVariables` may be claimed by at most one entry) and re-apply the runtime pipeline. `register` throws `StyleRegistryError` on an exclusive-field conflict.

**Additional methods:**

```typescript
styleRegistry.update(id: string, partial: Partial<Omit<StyleRegistryEntry, 'id'>>): void
styleRegistry.applyOverrides(overrides: Record<string, boolean>): void  // Called by AppSettingsProvider on bootstrap
styleRegistry.setOverride(id: string, value: boolean): void
styleRegistry.clearOverride(id: string): void
styleRegistry.getOverrides(): Record<string, boolean>
styleRegistry.isEnabled(id: string): boolean                    // Override wins over module default
styleRegistry.getActiveEntries(): StyleRegistryEntry[]          // Enabled entries, sorted by order
styleRegistry.getEffectiveComponentsJson(): ComponentsJsonContribution  // Merged across active entries
```

**Hook:** `useStyleRegistry()` → `StyleRegistryEntry[]`

For the live "currently-active" set (respecting overrides), call `styleRegistry.getActiveEntries()` directly — the hook tracks registered entries, not derived enabled state. See [Registry Reference](/starter-template/registries/) for the conceptual model.
...
// Execute actions
actionRegistry.doAction<C>(hook: string, context?: C): void           // Sync pool only — dev-warns (once per hook) when async handlers would be skipped (v1.54.0)
actionRegistry.doActionAsync<C>(hook: string, context?: C): Promise<void>  // Async pool only, sequential — dev-warns when sync handlers would be skipped (v1.54.0)
actionRegistry.doActionAll<C>(hook: string, context?: C): Promise<void>    // Both pools: sync then async sequentially (never warns)
actionRegistry.doActionParallel<C>(hook: string, context?: C): Promise<void> // Both pools: sync inline, async in parallel
...
**Import:** `../../config/route.registry.js` (backend, from a feature folder — the backend has no `@/` alias)
...
**Import:** `../../config/service.registry.js` (backend, from a feature folder)
...
**Import:** `../security/index.js` (backend, from a feature folder)

See [Registries — securityRegistry](/starter-template/registries/#securityregistry-backend) for usage. The handler types are in the [Types reference](/reference/types/#security-types).

```typescript
// Security manifests
securityRegistry.register(manifest: ModuleSecurityManifest): void
securityRegistry.getManifest(moduleId: string): ModuleSecurityManifest | undefined
securityRegistry.getAll(): ModuleSecurityManifest[]
securityRegistry.getOwnedModels(): OwnedModelEntry[]
securityRegistry.getEncryptedFields(): FieldEntry[]
securityRegistry.getHashedSecrets(): FieldEntry[]
securityRegistry.getDataCategories(): DataCategoryEntry[]
securityRegistry.getAuditEvents(): string[]
securityRegistry.getCredentialTypes(): string[]

// Deletion cascade
securityRegistry.registerDeletionHandler(handler: DeletionHandler): void
securityRegistry.getDeletionHandlers(): DeletionHandler[]   // sorted by order

// Personal-data export (v2.13.0)
securityRegistry.registerExportHandler(handler: DataExportHandler): void
securityRegistry.getExportHandlers(): DataExportHandler[]   // registration order

securityRegistry.findTierViolations(active: SecurityProfile): Array<{ moduleId: string; minimumTier: SecurityProfile }>
securityRegistry.clear(): void

// Run every export handler (also from the security barrel); never throws
collectDataExport(subject: DataExportSubject, opts?: { at?: Date }): Promise<DataExportCollection>
```

Both `register*Handler` methods skip a handler whose `id` is already registered.

---

### filterRegistry (Backend)

**Import:** `../../config/filter.registry.js` (backend, from a feature folder)
...
**Import:** `../../config/action.registry.js` (backend, from a feature folder)
