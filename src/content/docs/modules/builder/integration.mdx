...
const config = await builderService.getConfig('sharedOptions');
```
...
| `saveSharedOptionSet(name, options)` | `Record<string, unknown>` | Create/update shared options |
| `deleteSharedOptionSet(name)` | `Record<string, unknown>` | Delete shared options |
