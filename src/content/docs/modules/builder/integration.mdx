...
3. Compilation is lazy, memoised, and cycle-guarded; a source that fails to compile is skipped in isolation, so one bad component never blanks the preview. With no snapshot pinned, the preview uses the shared pristine component map.
