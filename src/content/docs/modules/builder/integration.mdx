...
| `promoteProjectComponentUpdate(projectId)` | `builder_component_set` | Promote the pending candidate to the live snapshot |
...
- **Management UI:** the **Component Sources** panel in the project's Blocks tab drives **Sync**, **Check for updates** (diff-only), **Re-sync**, and **Update files** (promote the pending candidate). Every route is scoped by project ownership.
...
The builder fires 8 CRUD action hooks from its backend service via `actionRegistry.doActionAll()`. Companion modules can subscribe to these hooks to react to builder lifecycle events — for example, updating analytics, triggering notifications, or syncing external systems.
...
All 8 hooks follow the same pattern — use `actionRegistry.addActionAsync()` with the hook name and a unique registration ID.
