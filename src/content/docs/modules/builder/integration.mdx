...
- **Storage:** the `builder_component_set` table (one row per project) holds `library`, the combined `style` key (e.g. `radix-luma`), `syncedAt`, the `files` map (`{ componentName: rewrittenSource }`), and an optional `pending` update candidate.
