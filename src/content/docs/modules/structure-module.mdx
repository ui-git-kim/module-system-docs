...
The installer creates default types based on your prompts. All types are active by default and can be modified in the admin panel or via code.
...
### Node Type Configuration Shape

The full configuration interface for a node type:
...
<Aside type="note" title="isSystem flag">
...
Control which entries appear in the sidebar for quick access:
...
The card preview component renders a mock card using the current form state. Toggle settings and watch the preview update instantly - no need to save first.
</Aside>
...
Admins configure the navigation style per node type in the Pages tab:
...
The Structure module uses the starter template's dynamic breadcrumb system. When viewing a node:
...
Each node type gets a list page at `/:nodeType` (e.g., `/projects`). The list page supports:
...
How cards display in grid/list views is controlled by the `card` configuration in the node type's `listPage` settings. Admins configure this visually in the Pages tab with the live card preview.
...
<Aside type="tip">
The tab, widget, and card extension registries all extend `BaseRegistry` from `@/config/base.registry`. This gives them standard methods like `register()`, `unregister()`, `get()`, `getAll()`, and automatic React hooks.
</Aside>
...
The `useEnabledNodeTypes` and `useStructureNavItems` hooks subscribe to the registry's `onSync` event. They automatically re-render when node types are updated from the database, ensuring the UI stays in sync.
</Aside>
...
**Node Components** (`components/node/`):

| Component | Description |
|-----------|-------------|
| `NodeIcon` | Renders the appropriate Lucide icon for a node |
| `NodeCard` | Card display for grid/list views, reads card config from node type |
...
| `CardPreview` | Live preview of node card using current form state |
...
| PATCH | `/types/reorder` | Update sort order for multiple types |
...
The following shadcn/ui components are installed automatically:

`alert` `alert-dialog` `badge` `button` `card` `collapsible` `command` `dialog` `dropdown-menu` `input` `label` `popover` `scroll-area` `select` `skeleton` `switch` `tabs` `textarea` `tooltip`

### npm Dependencies

| Package | Purpose |
|---------|---------|
| `react-hook-form` | Form state management for NodeTypeForm and NodeForm |
...
- [Registry Reference](/starter-template/registries/) - Full API for all starter registries
- [Lifecycle Hooks](/module-starter/lifecycle-hooks/) - Filters and actions for cross-module integration
