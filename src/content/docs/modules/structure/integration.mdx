...
When nodes are created, updated, deleted, moved, archived, published, duplicated, or when stages change, Structure fires **49 action hooks**. Your module listens to these events and reacts: clean up tasks when a project is deleted, generate an invoice when a client is archived, send a notification when a node is moved. This is the primary mechanism for **cross-module coordination**.
...
<CardGrid>
  <Card title="React to Events" icon="rocket">
    Hook into 49 lifecycle events — run logic when nodes are created, updated, deleted, moved, archived, published, duplicated, or connected.
  </Card>
...
Extend the field editor system with custom field types. Structure includes 22 built-in types; your module can add more — and a module-registered type overrides a built-in of the same id.
...
**Available Structure action hooks (49 total):**
...
| `structure.connections.bulkCreated` | `{ userId, count, connectionIds }` | Multiple connections are created in one bulk operation |
