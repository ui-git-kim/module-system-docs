...
When nodes are created, updated, deleted, moved, archived, published, duplicated, or when workflow states change, Structure fires **27 action hooks**. Your module listens to these events and reacts: clean up tasks when a project is deleted, generate an invoice when a client is archived, send a notification when a node is moved. This is the primary mechanism for **cross-module coordination**.
...
<CardGrid>
  <Card title="React to Events" icon="rocket">
    Hook into 27 lifecycle events — run logic when nodes are created, updated, deleted, moved, archived, published, duplicated, or connected.
  </Card>
...
**Available Structure action hooks (27 total):**
