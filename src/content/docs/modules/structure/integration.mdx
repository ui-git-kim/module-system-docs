...
Structure exposes **11 module-specific registries**. 8 are content registries (`nodeTabRegistry`, `nodeWidgetRegistry`, `nodeCardExtensionRegistry`, `nodeActionRegistry`, `nodeColumnRegistry`, `nodeFilterRegistry`, `nodeSidebarSectionRegistry`, `nodeFieldTypeRegistry`). Two are DB-synced configuration registries (`nodeTypeRegistry` for node types, `connectionTypeRegistry` for typed connections — each with a dedicated admin tab). The 11th is `structureRegistry`, which handles module field injection, lifecycle hooks, and delete validation.
</Aside>

### 2. React to Node Events (Backend Action Hooks)

When nodes are created, updated, deleted, moved, archived, published, duplicated, or when workflow states change, Structure fires **26 action hooks**. Your module listens to these events and reacts: clean up tasks when a project is deleted, generate an invoice when a client is archived, send a notification when a node is moved. This is the primary mechanism for **cross-module coordination**.
...
<CardGrid>
  <Card title="React to Events" icon="rocket">
    Hook into 26 lifecycle events — run logic when nodes are created, updated, deleted, moved, archived, published, duplicated, or connected.
  </Card>
...
| Register connection types | ✅ | ✅ | `connectionTypeRegistry` + `structureService` |
...
Extend the field editor system with custom field types. Structure includes 16 built-in types; your module can add more.
...
Extends the field type system with custom field types beyond the 16 built-in types.
...
**Available Structure action hooks (26 total):**
