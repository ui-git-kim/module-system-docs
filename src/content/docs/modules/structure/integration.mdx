...
When nodes are created, updated, deleted, moved, archived, published, duplicated, or when workflow states change, Structure fires **43 action hooks**. Your module listens to these events and reacts: clean up tasks when a project is deleted, generate an invoice when a client is archived, send a notification when a node is moved. This is the primary mechanism for **cross-module coordination**.
...
<CardGrid>
  <Card title="React to Events" icon="rocket">
    Hook into 43 lifecycle events — run logic when nodes are created, updated, deleted, moved, archived, published, duplicated, or connected.
  </Card>
...
Extend the field editor system with custom field types. Structure includes 21 built-in types; your module can add more — and a module-registered type overrides a built-in of the same id.
...
Extends the field type system with custom field types beyond the 21 built-in types.
...
**Available Structure action hooks (43 total):**
...
Structure ships opt-in cognitive foundations: pgvector node embeddings with a semantic search service, keyword (lexical) and hybrid search, and a `features.cognitiveIngest` flag on node types. Because embeddings are an install-time opt-in, probe availability with `structureService.getCognitiveCapabilities()` — it returns `{ embeddings: { enabled, dimensions }, chunks: { enabled }, lexical: { enabled }, graph: { enabled } }` without hard imports. `lexical.enabled` mirrors `embeddings.enabled` (the lexical and hybrid methods ride the same opt-in), while `graph.enabled` is always `true` because connection-nodes are core:
...
The `structureEmbeddings` service (registered only when the app opted into embeddings) provides `setNodeEmbedding`, `clearNodeEmbedding`, `listNodesNeedingEmbedding`, `semanticSearchNodes`, `semanticSearchChunks`, and `findSimilarNodes`, plus (v1.32.0+) the lexical and hybrid methods `lexicalSearchNodes`, `lexicalSearchChunks`, `hybridSearchNodes`, and `hybridSearchChunks`. The lexical methods are keyword search (Postgres full-text `websearch_to_tsquery` + `ts_rank_cd` over a GIN index — the `score` is a rank, not a `0..1` similarity), and the hybrid methods fuse the lexical and semantic arms with reciprocal rank fusion in SQL; the caller supplies **both** the query text and its embedding. Structure owns storage only — it never calls an LLM or embedder, so callers always supply the vectors and query text. It is an in-process API only — there are no HTTP routes for embeddings or search. See the [Cognitive guide](/modules/structure/cognitive/) for full signatures, the `features.cognitiveIngest` flag, and install/opt-in details.
