...
4. **Mount `<StructureInit />`** in your authenticated layout
   ...
   This is **required** if your app uses Structure's auto-generated sidebar navigation. `StructureInit` mounts `useNodeTypes()` internally so node-type nav items appear on first render post-login, not only after the user visits a Structure page.

5. **Initialise backend** in `server.ts`
   ```typescript
...
- **`<StructureInit />` belongs in a post-auth layout.** It calls the API on mount — placing it above the auth boundary produces a 401.
