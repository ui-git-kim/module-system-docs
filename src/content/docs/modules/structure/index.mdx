...
<CardGrid>
  ...
  <Card title="Extensible" icon="puzzle">
    Ten module registries — nine content registries plus `nodeTypeRegistry` and `connectionTypeRegistry` — let other modules add tabs, widgets, actions, columns, filters, sidebar sections, card extensions, field types, and typed connections.
  </Card>
...
- Node context provider for AI integration (ancestors, connections, breadcrumb, contextHints)
- Namespaced metadata isolation for safe cross-module data writes
- Path-based descendant queries for hierarchical scoping
...
- 22+ backend action hooks for cross-module event handling
...
Structure exposes **ten registries** — nine content registries plus `nodeTypeRegistry` and `connectionTypeRegistry` (both DB-synced with dedicated admin tabs). Other modules use these to extend nodes:
...
The service fires **22+ action hooks** after every mutation (node created, updated, deleted, moved, archived, published, workflow state changed, duplicated, bulk created, connection type CRUD, etc.) and applies **4 filter hooks** at key pipeline points (before create, response enrichment, query modification, node type response). Other modules subscribe to these hooks via the starter template's `actionRegistry` and `filterRegistry`.
...
**Node type field types:** `text`, `textarea`, `number`, `date`, `datetime`, `select`, `multiselect`, `toggle`, `url`, `email`, `secret`, `node-reference`, `media`
