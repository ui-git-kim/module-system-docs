...
<CardGrid>
  ...
  <Card title="Extensible" icon="puzzle">
    Nine module registries — eight content registries plus `nodeTypeRegistry` — let other modules add tabs, widgets, actions, columns, filters, sidebar sections, card extensions, and field types.
  </Card>
...
Structure exposes **nine registries** — eight content registries plus `nodeTypeRegistry` (which is DB-synced and has its own dedicated Structure Types admin tab). Other modules use these to extend nodes:

| Registry | Purpose |
|----------|---------|
| `nodeTypeRegistry` | Node type configurations (DB-synced; managed via the Structure Types admin tab) |
