...
- **Programmatic writers:** skip validation only for `editableBy:'system'` fields written by trusted service callers (cognitive-pipeline's graph-derived values) — reuse the `editableBy` classification the write path already reads.
...
> **Permission half shipped in v1.33.32**, with the per-type READ tier following in v1.34.12. Delivered **more** than the plan asked for: `permissions` now carries **five** keys — `read` / `create` / `edit` / `archive` / `delete` — each on the `'anyone' | 'admin' | 'system'` tier (`frontend/index.ts`); `'system'` means only trusted in-process callers (e.g. cognitive-pipeline), so no user or admin can perform it via the UI or API. Enforcement is in the **service methods** as planned, `attachRole` is now applied to **every** node-write route (POST `/nodes`, `/nodes/bulk`, bulk-archive, bulk-delete, bulk-restore, PATCH, DELETE, move, duplicate, archive, restore), and the `NodeTypeForm` permission editor exists on the Permissions section (`pages.ts`).
>
...
- **Cross-module:** add `rootNodeId` to node lifecycle payloads (`structure.node.created`, `.updated`, `.moved`) and connection payloads — consumed by cognitive-pipeline and document-management.
