...
| Export | Description |
|--------|-------------|
| `<StructureInit />` | **Required** for apps using Structure's sidebar navigation. Mount once inside your authenticated layout — renders nothing, mounts `useNodeTypes()` internally so node-type nav items appear on first render post-login (not only after the user visits a Structure page) |
| `initStructureNavigation()` | Imperative alternative to `<StructureInit />`. Call from a post-auth `useEffect` or app init hook if you need one-shot priming outside a React tree |
