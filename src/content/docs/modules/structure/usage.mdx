...
| Export | Description |
|--------|-------------|
| `initStructureNavigation()` | Optional — fetches node types and syncs the registry. Call from a post-auth init point in your app if you want navigation primed before any Structure page mounts. Most apps don't need it — `useNodeTypes()` on any Structure page syncs the registry automatically |
