...
| `src/templates/migrations.ts` | Raw SQL migration templates (applied after `prisma migrate dev`) |
...
The `version:*` scripts bump the version and create a commit. For a tagged release, use the `release:*` variants instead — they perform the same bump and commit, but also create a matching git tag:
