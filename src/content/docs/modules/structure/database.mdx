...
| `cardTemplateId` | String? | FK → structure_template.id for card view |
| `listItemTemplateId` | String? | FK → structure_template.id for list item view |
| `detailHeaderTemplateId` | String? | FK → structure_template.id for detail header |
| `overviewTemplateId` | String? | FK → structure_template.id for overview layout |
| `templateOverrides` | JSON | Local overrides on top of assigned templates |
...
Reusable rendering templates for views. A template defines how nodes render in a specific view (card, list, detail header, overview). Templates are standalone entities that can be assigned to multiple node types with local overrides.

| Column | Type | Description |
|--------|------|-------------|
| `id` | String | CUID primary key |
| `userId` | String | Owner |
| `name` | String | Display name (e.g., "Compact Card") |
| `slug` | String | URL-friendly identifier |
| `description` | String? | Template description |
| `type` | String | `card`, `listItem`, `detailHeader`, or `overview` |
| `config` | JSON | View-specific configuration (slots, visibility, extensions) |
...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/templates` | List all templates (optional `?type=` filter) |
...
| `updateTemplateSchema` | PATCH `/templates/:id` | All fields optional (partial of create schema) |
