...
| **cognitive-pipeline** | The pipeline config (Intake → Acquisition → Preprocessing → Extraction → Actions) + presentation (tabs/widgets) | How content comes in, gets parsed, and how results surface in the app. |
...
- Structure deliberately ships **no** search/embeddings today ("AI-discovered connections → cognitive-pipeline / cognitive-db") — so a vector column is a *new, opt-in* capability, not a conflict.
...
2. **Raw content** — a **field on the node**, not its own type. The uploaded court doc *is* a `structure_node` (type "Court Document") with its text in a field; cognitive-pipeline hands it to the engine to parse; it stays referable.
...
On ingest, cognitive-pipeline resolves the target Structure node (or type) and hands the parsed content to the engine. The engine then:
...
Working memory is a *generic, cross-module* concept: "what the user has been working on," their recent thought process, and the context a chat/LLM call draws on. **cognitive-pipeline registers it** — a `working-memory` Structure node type plus an action/filter hook surface (the starter's [hook registry](/starter-template/hooks/)) — so any module can *contribute to* or *react to* it without cognitive-pipeline knowing those modules exist.

### The node type
cognitive-pipeline registers a `working-memory` node type (via `structureRegistry` / node-type registration). Instances are Structure nodes (user-owned); granularity is app-defined (a rolling "current focus" per user, or one per session/chat). Shape:
...
### Hook surface (cognitive-pipeline is the provider; any module plumbs in)
**Filters** — modules *contribute* to a value (`filterRegistry.applyAll`, backend sync+async):
- `cognitive-pipeline.working-memory.assemble` — build the working set. Seeded, then every module appends its "what the user is working on" (a Tasks module adds active tasks; Documents adds recently-opened files; the engine adds semantically-relevant nodes). `applyAll('cognitive-pipeline.working-memory.assemble', items, { userId, sessionId })`.
- `cognitive-pipeline.working-memory.context` — shape the context handed to a chat/LLM (add / trim / rank, token-aware). This is the "context for chats" seam.

**Actions** — modules *react* (`actionRegistry.doActionAll`):
- `cognitive-pipeline.working-memory.updated` — `{ userId, workingMemoryId, change }`. The engine **consolidates** (learns from what got used); a chat refreshes its context.
- `cognitive-pipeline.working-memory.item.added` / `.item.removed` — active-set changes.
- `cognitive-pipeline.working-memory.session.started` / `.ended` — session lifecycle (if the app models sessions).

### Flow
1. Something needs context (a chat turn, or the user shifts focus).
2. cognitive-pipeline runs `filterRegistry.applyAll('cognitive-pipeline.working-memory.assemble', seed, ctx)` → modules + the engine contribute → the working set is reflected on the `working-memory` node (active-set connections).
3. For a chat, `filterRegistry.applyAll('cognitive-pipeline.working-memory.context', prompt, ctx)` lets modules finalise what goes into the prompt.
4. cognitive-pipeline fires `actionRegistry.doActionAll('cognitive-pipeline.working-memory.updated', ctx)` → the engine consolidates; modules react.

Working memory is user-facing (Structure holds it) but *composed* by many sources and *consumed* by many (chats, the engine). The hook surface is the starter's decoupling pattern applied: cognitive-pipeline owns the node type + the events; everyone else plugs in. The engine only *assembles* (semantic + graph) and *consolidates* — it never owns the state.
...
- Opt-in **embedding**: a nullable `vector` column + pgvector extension + HNSW index, provisioned when a cognitive app is detected (e.g. cognitive-pipeline installed / a Structure setting). Structure exposes a write path for the engine to set embeddings; semantic-search query helpers may live here or be issued by the engine.
...
**cognitive-db v2.0.0** (breaking) · **cognitive-pipeline** minor→major (pipeline output changes) · **Structure** minor (additive: flag, opt-in embedding, connection origin, proposal API, working-memory node type). Net result: **one record store (Structure), one intelligence layer (`cog_intelligence_*`), one pipeline (cognitive-pipeline) — zero record duplication.**
...
- Module: `C:/Users/kimbe/ReactApps/Cog/cognitive-pipeline`
- Structure: `C:/Users/kimbe/ReactApps/structure`
- This RFC's source: `cognitive-pipeline/docs/src/content/docs/cognitive-db/v2-rfc.md` (published under Libraries → cognitive-db → *v2 RFC*)
...
2. **Working-memory node type + hook surface** (§7) — also additive; cognitive-pipeline registers the `working-memory` type and the `cognitive-pipeline.working-memory.*` hooks. ✅ **Shipped in [cognitive-pipeline v1.12.0](/modules/cognitive-pipeline/working-memory/).**
...
4. **cognitive-pipeline rewire** (§9.3) — pipeline writes Structure nodes; make `cog_ingest_mapping` real; slim `cog_ingest_job`; content-as-field.
5. **Intelligence + retire v1** (§9.4–9.5) — patterns/questions as nodes; discovered-connection promotion; delete `cog_node`; cut **v2.0.0**.

**Also true:** cognitive-db is currently **v1.0.0** (audited, all 84 findings fixed) and vendored into cognitive-pipeline **v1.11.10**. Nothing here is in production, so breaking changes are fine.
